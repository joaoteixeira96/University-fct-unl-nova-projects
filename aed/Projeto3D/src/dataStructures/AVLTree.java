 ...
 *
 * @param <K>
 *            Generic type Key, must extend comparable
 * @param <V>
 *            Generic type Value
 */
public class AVLTree<K extends Comparable<K>, V> extends AdvancedBSTree<K, V> {
	...
	/**
	 * Every ancestor of the new leaf is stored in the stack, which is not
	 * empty.
	 * 
	 * @param path - Stack of PathStep objects containing all ancestors of the
	 *            inserted node
	 */
	 ...
	 */
	protected void rebalanceInsLeft(AVLNode<K, V> node, Stack<PathStep<K, V>> path) {
	...
	/**
	 * Every ancestor of the removed node is stored in the stack, which is not
	 * empty.
	 * 
	 * @param path - Stack of PathStep objects containing all ancestors of the
	 *            inserted node
		...
		while (decrease && parent != null) {
			if (lastStep.isLeftChild)
				...
				case 'E':
					...
					break;
				case 'R':
					this.rebalanceRemRight(parent, path, decrease);
					break;
				}
			else
				// parent's right subtree has grown.
				switch (parent.getBalance()) {
				case 'L':
					this.rebalanceRemLeft(parent, path, decrease);
					decrease = false;
					break;
				case 'E':
					parent.setBalance('L');
	...
	/**
	 * @param node
	 * @param path - root of subtree to balance
	 * @param decrease
	 */
	protected void rebalanceRemLeft(AVLNode<K, V> node, Stack<PathStep<K, V>> path, boolean decrease) {
		AVLNode<K, V> leftChild = (AVLNode<K, V>) node.getLeft();
		switch (leftChild.getBalance()) {
		case 'L':
			this.rotateLeft1L(node, leftChild, path);
	...
	protected void rebalanceRemRight(AVLNode<K, V> node, Stack<PathStep<K, V>> path, boolean decrease) {
		AVLNode<K, V> rightChild = (AVLNode<K, V>) node.getRight();
		switch (rightChild.getBalance()) {
		case 'L':
			this.rotateRight2(node, rightChild, path);
	...
	/**
	 * Performs a single left rotation rooted at theRoot, when the balance
	 * factor of its leftChild is 'L'. Every ancestor of theRoot is stored in
	 * the stack, which is not empty. height( node.getLeft() ) - height(
	 * node.getRight() ) = 2.
	 * 
	 * @param theRoot
	 *            - root of the rotation
	 * @param leftChild
	 *            - Left child of theRoot
	 * @param path
	 *            - Stack of PathStep objects containing all ancestors of
	 *            theRoot
	 */
	protected void rotateLeft1L(AVLNode<K, V> theRoot, AVLNode<K, V> leftChild, Stack<PathStep<K, V>> path) {
	...
	/**
	 * Performs a single left rotation rooted at theRoot, when the balance
	 * factor of its leftChild is 'E'. Every ancestor of theRoot is stored in
	 * the stack, which is not empty. height( node.getLeft() ) - height(
	 * node.getRight() ) = 2.
	 * 
	 * @param theRoot
	 ...
	 *            theRoot
	 */
	protected void rotateLeft1E(AVLNode<K, V> theRoot, AVLNode<K, V> leftChild, Stack<PathStep<K, V>> path) {
	...
	/**
	 * Performs a single right rotation rooted at theRoot, when the balance
	 * factor of its rightChild is 'R'. Every ancestor of theRoot is stored in
	 * the stack, which is not empty. height( node.getRight() ) - height(
	 * node.getLeft() ) = 2.
	 * 
	 * @param theRoot
	 *            - root of the rotation
	 * @param rightChild
	 *            - Right child of theRoot
	 * @param path
	 *            - Stack of PathStep objects containing all ancestors of
	 *            theRoot
	 */
	...
	/**
	 * Performs a single right rotation rooted at theRoot, when the balance
	 * factor of its rightChild is 'E'. Every ancestor of theRoot is stored in
	 * the stack, which is not empty. height( node.getRight() ) - height(
	 * node.getLeft() ) = 2.
	 * 
	 * @param theRoot
	...
	/**
	 * Performs a double left rotation rooted at theRoot. Every ancestor of
	 * theRoot is stored in the stack, which is not empty. height(
	 * node.getLeft() ) - height( node.getRight() ) = 2.
	 * 
	 * @param theRoot
	 *            - root of the rotation
	 * @param leftChild
	...
	/**
	 * Performs a double right rotation rooted at theRoot. Every ancestor of
	 * theRoot is stored in the stack, which is not empty. height(
	 * node.getRight() ) - height( node.getLeft() ) = 2.
	 * 
	 * @param theRoot
	 *            - root of the rotation
	 * @param rightChild
