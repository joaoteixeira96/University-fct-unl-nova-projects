...
public class OrderedDoubleList<K extends Comparable<K>, V> implements OrderedDictionary<K, V> {
	...
	protected void addFirst(DListNode<Entry<K, V>> newNode) {
		if (this.isEmpty())
			tail = newNode;
		else {
	...
	protected void addLast(DListNode<Entry<K, V>> newNode) {
		if (this.isEmpty())
			head = newNode;
		else {
	...
	protected void addMiddle(DListNode<Entry<K, V>> newNode, DListNode<Entry<K, V>> prevNode) {
	...
	public V insert(K key, V value) {
		...
		while (searchNode != null) {
			...
			if (key.compareTo(KSearch) == 0) {
				...
				V v= searchNode.getElement().getValue();
				searchNode.setElement(element);
				return v;
			}
